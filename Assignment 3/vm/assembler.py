...
# [/class]
    ...
    def _compile(self, instruction, labels):
        ...
        if fmt == "--":
            ...
            if code == "inc":
                return self._combine(self._reg(args[0]), 1, code)
            elif code == "dec":
                return self._combine(self._reg(args[0]), -1, code)
            else:
            ...
            if code == "swp":
                temp = copy.deepcopy(self._reg(args[0]))
                self._combine(self._reg(args[0]), self._reg(args[1]), "cpy")
                self._combine(self._reg(args[1]), temp, "cpy")
                del temp
                return
